...
export async function POST(request: NextRequest) {
  try {
    ...
    // 获取文件记录
    console.log('处理WhatsApp文件，ID:', fileId);
    ...
    await updateDbFileStatus(fileId, ChatFileStatus.PROCESSING, {
      platform: 'whatsapp' // 确保平台信息为whatsapp
    });

    // 使用通用文件处理服务处理文件
    try {
      await processUploadedFile(fileId, 'whatsapp', async (status) => {
        // 更新本地存储中的处理状态
      ...
      return NextResponse.json({
        success: true,
        message: 'WhatsApp 数据处理完成',
        fileId: fileId
      });
    } catch (error) {
      console.error('处理 WhatsApp 数据失败:', error);
      ...
      return NextResponse.json(
        {
          success: false,
          message: error instanceof Error ? error.message : '处理 WhatsApp 数据失败'
        },
        { status: 400 }
      );
    }
  } catch (error) {
    console.error('处理 WhatsApp 请求失败:', error);

    return NextResponse.json(
      { success: false, message: '处理 WhatsApp 请求失败' },
      { status: 500 }
    );
  }
}
