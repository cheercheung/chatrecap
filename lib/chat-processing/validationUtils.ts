...
import { stringSimilarity } from '@/utils/textUtils';
...
export function removeDuplicates(messages: RawMessage[]): RawMessage[] {
  ...
  // 过滤掉 undefined 或 null 的消息
  const validMessages = messages.filter(message => message != null);

  for (const message of validMessages) {
    // 确保消息有所有必要的字段
    if (!message.timestamp || !message.sender || !message.message) {
